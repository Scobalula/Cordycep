...
ps::LZ4DecompressorV2::LZ4DecompressorV2(ps::FileHandle& file, bool secure) :
	Decompressor(file, secure),
	DistanceToNextHashBlock(SIZE_MAX)
{
}

size_t ps::LZ4DecompressorV2::ReadSecure(uint8_t* buffer, const size_t size)
{
...
size_t ps::LZ4DecompressorV2::Decompress(uint8_t* input, size_t inputSize, uint8_t* output, size_t outputSize)
{
	return LZ4_decompress_safe((const char*)input, (char*)output, (int)inputSize, (int)outputSize);
}

size_t ps::LZ4DecompressorV2::Read(void* ptr, const size_t size, const size_t offset)
{
